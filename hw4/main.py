...
def update_params_for_one_item(K, n_word_types, word_indices, counts, gammas, phi_d, log_betas, alpha, max_iter_d,
    ...
    while i < max_iter_d and delta >= tol_d:
        ...
        while n < n_word_types:
            ...
            ## Your codes here.
            for j in range(K):
                phi_d[n, j] = math.exp(log_betas[w][j] + psi_gammas[j])
        
            # normalize phi
            sum_phi = np.sum(phi_d, axis=1)[n]
            phi_d[n] /= sum_phi

            n += 1

        # update gamma
        gamma = np.full(K, alpha)
        for i in range(n_word_types):
            gamma += phi_d[i]
        
        psi_gammas = psi(gamma)
...
def compute_bound_for_one_item(K, n_word_types, word_indices, count_vector, alpha, gammas, psi_gammas, phi_d,
    ...
    ## Your codes here.
    bound = gammaln(alpha * K) - K * gammaln(alpha)

    sum_psi = np.sum(psi_gammas)
    sum_gammas = np.sum(gammas)
    psi_sum_gamma = psi(np.sum(gammas))

    bound += (alpha - 1) * (sum_psi - K * psi_sum_gamma)

    for n in range(n_word_types):
        for i in range(K):
            bound += phi_d[n][i]*(psi_gammas[i]-psi_sum_gamma + log_betas[word_indices[n], i] - math.log(phi_d[n, i]))*count_vector[n]

    bound -= gammaln(sum_gammas)
    for i in range(K):
        bound += gammaln(gammas[i]) - (gammas[i]-1)*(psi_gammas[i]-psi_sum_gamma)
